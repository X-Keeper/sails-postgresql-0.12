...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function(connection, collections, cb) {
      ...
      async.map(_.keys(collections), function(collName, cb){
        ...
        // err is called for the first error of the describe calls,
        // but remaining functions will continue to run, so `endPool` is waiting for remaining connections
        // before calling `callback`.
        var failedConnection = connections[connection.identity];
        endPool(failedConnection, function() {
          // for example the `registerConnection` method called again, and it will create another
          // connection already (not that for which we call `endPool`)
          if(connections[connection.identity] === failedConnection) {
    ...
    teardown: function (conn, cb) {
      ...
      if (conn === null) {
        var toEnd = _.values(connections);
        connections = {};
        return async.each(toEnd, endPool, function() {
          cb();
        });
      }
      if(!connections[conn]) {
      ...
      var connectionObject = connections[conn];
      delete connections[conn];
      endPool(connectionObject, function() {
        cb();
      });
    },
    ...
    describe: function(connectionName, table, cb) {
      spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
        ...
        // Build a version-independent catalog query with only the fields used
        // by normalizeSchema().
        var query = 'SELECT x.attname as "Column", x."Type", r.conname as "Constraint", r.contype as "C" FROM (' +
          'SELECT c.oid, a.attnum, n.nspname, c.relname, a.attname, ' +
          ...
          'and c.relkind not in (\'S\',\'v\') and c.relnamespace = n.oid ' +
          'and n.nspname not in (\'pg_catalog\',\'pg_toast\',\'information_schema\')) x ' +
          'left join pg_constraint r on r.conrelid = x.oid and r.conkey[1] = x.attnum ' +
          'where x.relname = \'' + tableName + '\' and x.nspname = \'' + schemaName + '\' ' +
          'order by x.attnum;';
        ...
        client.query(query, function(err, result) {
          ...
          client.query(autoIncrementQuery, function(err, aResult) {
            ...
            client.query(indiciesQuery, function(err, iResult) {
              ...
              // Preserve the physical PostgreSQL type on the adapter-owned
              // schema used by waterline-sequel.  This happens after Waterline
              // normalization, so metadata such as `nativeType: 'uuid'` is not
              // stripped before criteria are compiled.
              utils.applyNativeTypes(connectionObject.schema, tableName, normalizedSchema);
    ...
    stream: function(connectionName, table, options, stream) {
      ...
      // Pass a callback: the promise form of `connect()` would reject with
      // nobody listening if the server is unreachable.
      client.connect(function(connectErr) {
        if(connectErr) {
          stream.end();
        }
      });
      ...
      // Run Query. Since pg 7 `client.query()` no longer returns a row-emitting
      // handle, but submitting a `pg.Query` instance still does.
      var dbStream = client.query(new pg.Query(_query.query[0], _query.values[0]));
  ...
  // Build the client pool backing a registered connection.
  function createPool(config) {

    var poolConfig;

    // If the connection details were supplied as a URL, pass it through as a
    // connection string and keep the remaining options as typed object values.
    if(_.has(config, 'url')) {
      poolConfig = _.omit(config, 'url');
      poolConfig.connectionString = config.url;
    } else {
      poolConfig = _.clone(config);
    }

    // `poolSize` is what pg 4.x called the cap that pg-pool names `max`.
    // Keep the documented adapter option working.
    if(_.has(config, 'poolSize') && !_.has(config, 'max')) {
      poolConfig.max = config.poolSize;
    }

    var pool = new pg.Pool(poolConfig);

    // An idle client dropping its socket (server restart, network blip) emits
    // on the pool. Without a listener that is an unhandled 'error' event and
    // node tears the process down.
    pool.on('error', function(err) {
      console.error('Postgresql pool error on an idle client:', err.message);
    });

    return pool;
  }

  // Drain a connection's pool. Errors here are not actionable -- teardown must
  // not fail because a socket had already gone away.
  function endPool(connectionObject, cb) {
    if(!connectionObject || !connectionObject.pool) {
      return cb();
    }

    try {
      connectionObject.pool.end(function() {
        cb();
      });
    } catch (e) {
      cb();
  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    // Grab a client instance from the client pool. The pool itself is built
    // once per connection in `registerConnection` -- pg >= 7 dropped the
    // `pg.connect()` helper that used to keep a global pool keyed by config.
    ...
    function after(err, client, done) {
      // pg-pool omits the release callback on some failure paths (notably
      // `connectionTimeoutMillis`), so never assume it was handed to us.
      done = done || function() {};
