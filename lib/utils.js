...
/**
 * Apply physical database types to the schema used by waterline-sequel.
 *
 * Waterline intentionally normalizes model attributes to portable logical
 * types (for example, a PostgreSQL UUID column is represented as `string`).
 * The adapter's `describe()` query has the authoritative physical types, so
 * copy those onto the adapter-owned schema after Waterline normalization.
 *
 * @param {Object} schema
 * @param {String} tableName
 * @param {Object} nativeSchema
 * @return {Array} physical column names that were matched
 */
utils.applyNativeTypes = function(schema, tableName, nativeSchema) {
  var tableSchema = schema && schema[tableName];

  if(!tableSchema) {
    tableSchema = _.find(schema, {tableName: tableName});
  }

  if(!tableSchema || !tableSchema.definition || !nativeSchema) {
    return [];
  }

  var matchedColumns = [];

  _.each(tableSchema.definition, function(attribute, attributeName) {
    if(_.isString(attribute)) {
      attribute = tableSchema.definition[attributeName] = {type: attribute};
    }
    ...
    var nativeColumn = nativeSchema[columnName];

    if(!nativeColumn || !_.isString(nativeColumn.type)) {
      return;
    }

    attribute.nativeType = nativeColumn.type;
